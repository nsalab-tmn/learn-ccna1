...
<!--
Схема представляет собой топологию сети, показывающую IPv4 и IPv6 адресацию сетевых устройств. Далее приводится описание топологии слева направо.  PC1 подключен к коммутатору, подключенному к маршрутизатору R1. Сетевой адрес IPv4 — 192.168.10.0/24, а адрес IPv6 — 2001:db8:acad:10: :/64. PC1 имеет адреса 0,10 и:: 10. интерфейс G0/0/0 на R1 имеет адрес .1 и ::1. Интерфейс R1 G0/0/1 затем подключается к маршрутизатору R2 в сети IPv4 209.165.200.224/30 и сети IPv6 2001:db8:feed:224: :/64. Интерфейс G0/0/1 на R1 имеет адрес .225 и: :1. Интерфейс на R2 имеет адреса .226 и: :2. R2 затем подключается к коммутатору, который подключен к PC2 в сети IPv4 10.1.1.0/24 и IPv6 сети 2001:db8:cafe:1: :/64. Интерфейс R2 имеет адреса .1 и: :1. PC2 имеет адреса 0,10 и:: 10. Маршрутизатор R2 подсоединен к Интернет-облаку.
-->
...
Используйте эту проверку синтаксиса для настройки интерфейса GigabitEthemet 0/0 на маршрутизаторе.

* Задайте имя канала как 'Link to LAN'.
* Настройте IPv4-адрес 192.168.10.2 и маску подсети 255.255.255.0.
* Configure the IPv6 address as 2001:db8:acad:10: :1 с длиной префикса /64.
