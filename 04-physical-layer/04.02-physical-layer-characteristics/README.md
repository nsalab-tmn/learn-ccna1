...
В предыдущей теме вы получили общий обзор физического уровня и его места в сети. В этой теме мы немного глубже погрузимся в специфику **физического уровня**. Мы рассмотрим компоненты и носители, используемые для построения сети, а также стандарты, необходимые для того, чтобы все работало вместе.

Протоколы и операции вышестоящих уровней модели OSI реализованы в программном обеспечении, созданном его разработчиками и компьютерными специалистами. Службы и протоколы в стеке протоколов TCP/IP определяются Инженерной группой по развитию Интернета (IETF).
...
Кроме этого, нередко местные спецификации разрабатываются региональными группами по кабельным стандартам. Например, Канадская ассоциация по стандартизации (CSA), Европейский комитет электротехнической стандартизации (CENELEC) и Японская ассоциация по стандартизации (JSA/JIS).
...
Производительность сети, состоящей из нескольких сетей или сегментов, не может превышать скорость самого медленного соединения между источником и получателем. Даже если все или большинство сегментов имеют высокую пропускную способность, один-единственный сегмент с низкой производительностью станет слабым звеном и производительность всей сети будет снижена.

**Производительность**

**Производительность** (throughput) — это количество битов, передаваемых по средствам подключения за определенный период времени.

Из-за множества факторов производительность обычно не соответствует заявленной пропускной способности в реализациях на физическом уровне. Производительность способность обычно ниже, чем пропускная способность. Есть три фактора, которые влияют на производительность:

* объем трафика;
* тип трафика;
* суммарная задержка, зависящая от количества сетевых устройств между источником и пунктом назначения.

Существует множество сервисов проверки скорости, позволяющих узнать реальную производительность интернет-соединения. На рисунке показан пример результата тестирования скорости.

**Полезная пропускная способность**

Существует также третий параметр, характеризующий передачу полезных данных, который называется **полезной пропускной способностью** (goodput). Полезная пропускная способность — это объем полезных данных, передаваемых за определенный период времени. Полезная пропускная способность равна производительности за вычетом служебного трафика, необходимого для создания сеансов, подтверждений, инкапсуляции и повторной передачи битов. Полезная пропускная способность всегда ниже производительности, которая, как правило, ниже пропускной способности.
