...
Часть 1. Подключиться к облаку

Часть 2. Подключить роутер Router0

Часть 3. Подключить оставшиеся устройства

Часть 4. Проверить подключения

Часть 5. Изучить физическую топологию
...
2.  Выберите правильный кабель для подключения порта **F0/0 Router0** к порту **Eth6 Cloud**. **Cloud** — это тип коммутатора, поэтому используйте подключение **Copper Straight-Through** (медное прямое). После подключения правильного кабеля индикатор канала на кабеле загорится зеленым.
...
После подключения правильного кабеля индикатор канала на кабеле загорится зеленым.

### Часть 2. Подключение роутера Router0
...
Выберите правильный кабель для подключения порта **F0/1 Router0** к порту **F0 netacad.pka**. Роутеры и компьютеры обычно используют одинаковые провода для отправки (1 и 2) и получения (3 и 6) данных. Кабель, который нужно выбрать, состоит из скрученных проводов. Хотя многие современные сетевые платы могут автоматически определить, какие пары используются для приема и передачи, на роутере **Router0** и сервере **netacad.pka** нет сетевых плат с этой функцией автоопределения.
...
После подключения правильного кабеля индикатор канала на кабеле загорится зеленым. Подождите несколько секунд, чтобы индикатор из оранжевого стал зеленым.
...
2.  Откройте **браузер** и введите адрес **http://netacad.pka**.
