...
Ping и traceroute — это две незаменимые команды для проверки TCP/IP-соединения. Ping — это утилита сетевого администрирования, которая проверяет доступность усторйства в IP-сети. Эта утилита также измеряет время, необходимое для отправки сообщений с исходного узла на узел назначения и обратно.

Утилита traceroute — это инструмент сетевой диагностики, который отображает путь или маршрут пакета и измеряет задержки передачи пакетов, проходящих по IP-сети.

В этой лабораторной работе в режиме симуляции физического оборудования рассматриваются команды **ping** и **traceroute** и изучаются параметры командной строки, влияющие на ход выполнения команд. Для их изучения в лабораторной работе используются компьютеры и устройства Cisco. Доступные параметры команд **ping** и **tracert** ограничены в Packet Tracer В лабораторной работе даются необходимые конфигурации для устройств Cisco.

## Инструкции

### Часть 1. Базовая проверка сети с помощью команды ping

В этой части лабораторной работы вы проверите сквозное соединение с помощью команды **ping**. Утилита ping отправляет на целевой узел пакеты с эхо-запросом протокола межсетевых управляющих сообщений (Internet Control Message Protocol, ICMP) и ждет ответа ICMP. Утилита фиксирует как суммарное время прохождения сигнала в прямом и обратном направлениях, так и потерю пакета.

У IP-пакетов ограниченный срок службы в сети. Пакеты IPv4 используют 8-битное время жизни (TTL). Пакеты IPv6 используют значение поля заголовка Hop Limit. TTL и Hop Limit определяют максимальное количество переходов уровня 3, которые могут быть пройдены на пути к месту назначения. Узлы в сети будут устанавливать собственное 8-битное значение с максимальным значением 255.

Каждый раз, когда IP-пакет поступает на сетевое устройство третьего уровня, это значение уменьшается на единицу, прежде чем он будет перенаправлен к месту назначения. Поэтому, если это значение в конечном итоге достигнет нуля, IP-пакет отбрасывается.
...
1.  Отправьте эхо-запрос с **PC-A** на шлюз по умолчанию, используя адрес IPv4 (интерфейс R1 GigabitEthernet 0/0/1).
    ...
    В этом примере отправили четыре запроса ICMP по 32 байта каждый. Ответы получили менее чем за одну миллисекунду без потери пакетов. Время передачи запросов и получения ответов растет по мере увеличения количества устройств, обрабатывающих запросы и ответы ICMP в процессе их передачи на узел назначения и обратно.
...
В этой таблице вы можете записать среднее время пути туда и обратно и TTL/Hop для пункта назначения. Введите свои ответы в пустые ячейки.
...
1.  Введите команду **tracert 209.165.200.226** в командной строке **PC-A**.
    ...
    **Примечание.** Вы можете остановить трассировку, нажав **Ctrl-C**.
    ...
    Результаты **Tracert** показывают, что путь от PC-А к External — это путь от PC-А к маршрутизатору R1 и провайдеру, и он не может достичь External. Результаты **tracert** указывают на проблему с маршрутизатором поставщика услуг Интернета.
...
Чтобы их посмотреть дополнительные параметры команды **traceroute**, после её ввода в командной строке поставьте знак вопроса **?** или просто нажмите клавишу **ввода**.
...
**Шаг 1. Получите доступ к сетевому расположению, где возникает проблема с подключением.**

На основе предыдущих шагов вы определили, что существует проблема на маршрутизаторе **ISP** с помощью команд **ping** и **traceroute**. У вас есть удаленный SSH доступ ко всем сетевым устройствам с использованием имени пользователя **admin** и пароля **class**.

1.  С терминала коммутатора **S1** подключитесь по SSH к маршрутизатору **ISP**, используя интерфейс G0/0/0 для устранения проблемы.
...
2.  С помощью параметра **–t** попробуйте установить соединение с PC-C, чтобы убедиться, что PC-C доступен. Параметр **-t** будет непрерывно отпралять эхо-запрос на цель пока вы сами не остановите исполнение этой команды. Используйте сочетание клавиш **Ctrl + C**, чтобы остановить эхо-запрос.
...
7.  Вышеуказанные шаги можно повторить для IPv6-адреса, чтобы получить сообщения об ошибке ICMP.
...
1.  От **R1** запустите эхо-запрос до **External**во внешней сети используя IP вдрес 209.165.200.226.
    ...
    Восклицательный знак (**!**) указывает, что эхо-запрос был успешным от маршрутизатора **R1** до **External**. Путь туда и обратно занимает в среднем 1 мс без потери пакетов, о чем свидетельствует 100% коэффициент успеха.
...
4.  Если в сети возникают проблемы, можно отправить расширенный эхо-запрос. Отправьте команду **ping** на адрес 209.165.200.226 с числом повторов 50000. Затем выключите интерфейс GigabitEthernet 0/0/1 на маршрутизаторе **ISP**.
    ...
    В сеансе SSH к **ISP** на коммутаторе **S1** отключите интерфейс GigabitEthernet 0/0/1 на **ISP**.
