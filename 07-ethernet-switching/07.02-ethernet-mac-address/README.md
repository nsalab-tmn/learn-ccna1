...
В сети IPv4-адреса представлены с использованием десятичной системы счисления с десятичной базой и двоичной системы счисления с основанием 2. Адреса IPv6 и Ethernet представлены с использованием базовой шестнадцатеричной системы счисления (HEX). Чтобы понять шестнадцатеричную систему, вы должны сначала познакомиться с двоичной и десятичной.

В шестнадцатеричной системе счисления используются цифры от 0 до 9 и буквы от A до F.
...
* использовать свой назначенный OUI в качестве первых 6 шестнадцатеричных цифр;
* назначить уникальное значение в последних 6 шестнадцатеричных цифрах.

Таким образом MAC-адрес Ethernet состоит из 6-ки шестнадцатеричного кода OUI поставщика, за которым следует 6 шестнадцатеричных значений, назначенных поставщиком, как показано на рисунке.
...
Поставщик несет ответственность за то, чтобы ни одному из его устройств не был присвоен одинаковый MAC-адрес. Тем не менее, дубликаты MAC-адресов могут существовать из-за ошибок, допущенных во время производства и в некоторых методах реализации виртуальных машин, или изменений, сделанных с помощью одного из нескольких программных средств. В любом случае MAC-адрес необходимо будет изменить в новой сетевой плате или программном обеспечении.
...
Любое устройство, которое является источником или адресатом кадра Ethernet, будет иметь сетевой адаптер Ethernet и, следовательно, MAC-адрес. К ним относятся рабочие станции, серверы, принтеры, мобильные устройства и роутеры.
...
Просмотрите, как обрабатывается одноадресный кадр. В этом примере MAC-адрес и IP-адрес назначения являются одноадресными.
...
Кадр широковещательной передачи Ethernet принимается и обрабатывается каждым устройством в локальной сети Ethernet. Рассмотрим функции широковещательной сети Ethernet.
...
Кадр многоадресной передачи Ethernet принимается и обрабатывается группой устройств в локальной сети Ethernet, принадлежащих к той же группе многоадресной рассылки. Рассмотрим функции многоадресной рассылки Ethernet.
...
* Он не пересылается роутером, если тот не настроен на маршрутизацию многоадресных пакетов.
...
Просмотрите, как обрабатывается многоадресный кадр. В этом примере MAC-адрес и IP-адрес назначения являются многоадресными.

![](./assets/7.2.6.png)

Протоколы маршрутизации и другие сетевые протоколы используют многоадресную адресацию. Такие приложения, как программное обеспечение для обработки видео и изображений, также могут ее использовать, хотя многоадресные приложения встречаются не так часто.
