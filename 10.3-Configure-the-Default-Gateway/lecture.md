...
<!--
Схема представляет собой топологию сети с одним маршрутизатором, двумя коммутаторами и четырьмя компьютерами, показывающими поток информации между устройствами в одной сети. PC1 и PC2 подключены к коммутатору в сети 192.168.10.0/24 к интерфейсу  G0/0/0 на R1. PC3 и PC4 подключены к другому коммутатору в сети 192.168.11.0/24 на интерфейсе G0/0/1 на R1. Стрелка показывает поток информации, отправляемой с PC1, проходящей через подключенный переключатель на пути к PC2.
-->
...
<!--
Схема представляет собой топологию сети с одним маршрутизатором, двумя коммутаторами и четырьмя компьютерами, показывающими поток информации между устройствами в разных сетях. PC1 и PC2 подключены к коммутатору в сети 192.168.10.0/24 к интерфейсу  G0/0/0 на R1. PC3 и PC4 подключены к другому коммутатору в сети 192.168.11.0/24 на интерфейсе G0/0/1 на R1. Стрелка показывает поток информации, отправляемой с PC1, проходящей через R1 и на PC3.
-->
...
<!--
Схема представляет собой топологию сети, показывающую маршрутизатор R1, подключенный к двум коммутаторам: S1 в сети 192.168.10.0/24 и S2 в сети 192.168.11.0/24. Пользователь подключен к S2, и стрелка показывает, что пользователь получает доступ к S1 удаленно. Над пользователем находится поле, показывающее, что пользователь имеет доступ CLI к S1 и отображает текущую конфигурацию.
-->
