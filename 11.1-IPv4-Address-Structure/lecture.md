...
<!--
На рисунке показана разбивка IPv4 адреса на сетевую и хостовую части. Адрес IPv4 — 192.168.10.10. Адрес преобразуется в 11000000 10101000 00001010 00001010. Пунктирная линия показывает разделение между сетевой и хостовой частью адреса. Это происходит после третьего октета и 24-го бита.
-->
...
<!--
На диаграмме показан процесс логического И между адресом узла IPv4 и маской подсети, приводящий к вычислению сетевого адреса IPv4-узла. Адрес узла IPv4 — 192.168.10.10. Ниже, адрес преобразуется в 11000000 10101000 00001010 00001010. Ниже записана маска подсети 255.255.255.0. Ниже этого маска подсети преобразуется в 11111111 11111111 111111 00000000. Линия рисуется под двоичным эквивалентом маски подсети. Под строкой находится десятичный и двоичный эквивалент сетевого адреса IPv4, определяемого процессом логического И. Синий затененный квадрат показывает первый бит адреса узла IPv4, 1, по сравнению с первым битом маски подсети, а также 1, в результате чего 1 в качестве первого битового значения в сетевом адресе IPv4. Сетевой адрес IPv4 — 192.168.10.0 с двоичным эквивалентом 11000000 101001000 00001010 00000000.
-->
...
<!--
Схема представляет собой топологию сети с четырьмя узлами, подключенными к коммутатору, подключенному к маршрутизатору. Интерфейс маршрутизатора имеет IP-адрес 192.168.10.1/24, а узлы имеют следующие IP-адреса: 192.168.10.10/24, 192.168.10.55/24, 192.168.10.101/24 и 192.168.10.12/24. Четвертый октет интерфейса маршрутизатора и хостов отображается в другом цвете. Круг включает интерфейс маршрутизатора, коммутатор и все узлы, внутри которых записывается сетевой адрес 192.168.10.0/24, а также четвертый октет показан другим цветом.
-->
