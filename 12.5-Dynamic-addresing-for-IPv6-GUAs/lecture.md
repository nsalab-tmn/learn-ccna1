...
![](./assets/12.5.1.png)
<!-- /courses/itn-dl/aeed55b2-34fa-11eb-ad9a-f74babed41a6/af2359d4-34fa-11eb-ad9a-f74babed41a6/assets/2e487694-1c25-11ea-81a0-ffc2c49b96bc.svg -->
...
<!--
На рисунке показана локальная сеть с хостом, отправляющим сообщение RS к маршрутизатору, а маршрутизатор посылает сообщение RA в ответ на ПК. Также в локальной сети находится DHCPv6 сервер. Текст под рисунком гласит 1. Сообщения RS отправляются на все маршрутизаторы IPv6 хостами, запрашивающими информацию об адресации.  2. Сообщения RA отправляются на все узлы IPv6. Если используется метод 1 (только SLAAC), RA включает сведения о префиксе, длине рефикса и шлюзе по умолчанию.
-->
...
![](./assets/12.5.2.png)
<!-- /courses/itn-dl/aeed55b2-34fa-11eb-ad9a-f74babed41a6/af2359d4-34fa-11eb-ad9a-f74babed41a6/assets/2e48c4b2-1c25-11ea-81a0-ffc2c49b96bc.svg -->
...
<!--
На рисунке показана локальная сеть с маршрутизатором, отправляющим сообщение объявления маршрутизатора ICMPv6 на ПК. ПК имеет адрес IPv6 2001:db8:acad:1:fc99:47ff:fe75:cee0/64. Рисунок, показывающий, что сетевой префикс, полученный в сообщении RA, равен 2001:db8:acad:1: а идентификатор интерфейса, который был создан клиентским устройством EUI-64 или случайным 64-битным числом, — fc99:47ff:fe75:cee0. Текст под рисунком гласит 1. Маршрутизатор отправляет сообщение RA с префиксом для локального канала. 2. ПК использует SLAAC для получения префикса из сообщения RA и создает свой собственный идентификатор интерфейса.
-->
...
![](./assets/12.5.3.png)
<!-- /courses/itn-dl/aeed55b2-34fa-11eb-ad9a-f74babed41a6/af2359d4-34fa-11eb-ad9a-f74babed41a6/assets/2e48ebc3-1c25-11ea-81a0-ffc2c49b96bc.svg -->
...
<!--
На рисунке показана локальная сеть с ПК, который отправляет маршрутизатору сообщения RS (labled #1). Маршрутизатор отправляет в ответ сообщение RA (labled #2). ПК также отправляет сообщение запроса DHCPv6 (labled #3) на сервер DHCPv6 без состояния. Текст под рисунком гласит 1. ПК отправляет RS на все маршрутизаторы IPv6, мне нужна информация об адресации. 2. Маршрутизатор отправляет сообщение RA всем узлам IPv6 с указанным способом 2 (SLAAC и DHCPv6). «Вот информация о префиксе, длине префикса и шлюзе по умолчанию, но вам нужно получить еще данные DNS (DNS-адрес) с DHCPv6-сервера». 3. ПК отправляет сообщение запроса DHCPv6 на все серверы DHCPv6. Я использовал SLAAC для создания адреса IPv6 и получения адреса шлюза по умолчанию, но мне нужна другая информация с сервера DHCPv6 без состояния.
-->
...
![](./assets/12.5.4.png)
<!-- /courses/itn-dl/aeed55b2-34fa-11eb-ad9a-f74babed41a6/af2359d4-34fa-11eb-ad9a-f74babed41a6/assets/2e4912d5-1c25-11ea-81a0-ffc2c49b96bc.svg -->
...
<!--
На рисунке показана локальная сеть с ПК, который отправляет маршрутизатор (labled #1). Маршрутизатор отправляет сообщение (labled #2) на компьютер. ПК также отправляет сообщение (labled #3) на сервер. Текст под рисунком гласит 1. ПК отправляет RS на все маршрутизаторы IPv6, мне нужна информация об адресации. 2. Маршрутизатор отправляет сообщение RA всем узлам IPv6 с указанным способом 3 (statefull DHCPv6). Я являюсь шлюзом по умолчанию, но вам нужно запросить информацию адресата IPv6 на сервер DHCPv6 с полным состоянием. 3. ПК отправляет сообщение запроса DHCPv6 на все серверы DHCPv6, я получил мой адрес шлюза по умолчанию из сообщения RA, но мне нужен адрес IPv6 и вся другая информация об адресации с сервера DHCPv6 с сохранением состояния.
-->
...
![](./assets/12.5.5.png)
<!-- /courses/itn-dl/aeed55b2-34fa-11eb-ad9a-f74babed41a6/af2359d4-34fa-11eb-ad9a-f74babed41a6/assets/2e4960f3-1c25-11ea-81a0-ffc2c49b96bc.svg -->
...
![](./assets/12.5.6.png)
<!-- /courses/itn-dl/aeed55b2-34fa-11eb-ad9a-f74babed41a6/af2359d4-34fa-11eb-ad9a-f74babed41a6/assets/2e49d621-1c25-11ea-81a0-ffc2c49b96bc.svg -->
...
<!--
На графике показаны этапы процесса EUI-64. Вверху графика находится MAC-адрес fc: 99:47:75:ce:e0. На шаге 1: Разделите MAC-адрес, первые 24-биты OUI fc: 99:47 в двоичном формате 1111 1100 1001 1001 0100 0111. Окончательные 24 бита идентификатора устройства 75:ce:e0 в двоичном формате 1111 0101 1100 1110 1110 0000. Шаг 2: Вставка ffee двоичное представление становится 1111 1100 1001 1001 0100 0111 1111 1111 1111 1111 1111 0101 1100 1110 1110 1110 0000. На третьем шаге Отразить u/l бит. 7-й бит слева изменяется с 0 на 1. Адрес в двоичном формате теперь 1111 1110 1001 1001 0100 0111 1111 1111 1111 1111 1111 0101 1100 1110 1110 1100 1110 0000. Измененный идентификатор интерфейса EUI-64 в шестнадцатеричном формате fe:99:47:ff:fe:75:ce:e0. Тест под графиком гласит: Шаг 1: Разделите MAC-адрес между OUI и идентификатором устройства. Шаг 2. Вставьте шестнадцатеричное значение FFFE в двоичном формате 1111 1111 1111 1110. Шаг 3. Преобразуйте первые 2 шестнадцатеричных значения OUI в двоичный формат и отразите бит U/L (бит 7). В данном примере 0 в седьмом бите меняется на единицу. В результате генерируется следующий EUI-64 идентификатор интерфейса FE99: 47FF:FE75:CEE0. Примечание. Использование обратного бита (U/L) и причины зеркального отражения его значения описаны в документе RFC 5342.
-->
