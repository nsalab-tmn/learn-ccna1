...
<!--
На рисунке показан адрес IPv6, разделенный на 64-битный префикс и 64-битный идентификатор интерфейса. 64-разрядный префикс: 2001:0db8:000a:0000. Идентификатор 64-битного интерфейса — 0000:0000:0000.
-->
...
<!--
На графике показана диаграмма из шести типов одноадресных адресов IPv6. Сверху вниз, типы Ipv6-адресов на диаграмме: Глобальный индивидуальный адрес, Глобальный индивидуальный адрес, Обратная петля ::1/128, Неопределенный адрес ::/128, Уникальный локальный адрес fc00::/7 - fdff::/7 и Встроенный IPv4-адрес.
-->
...
<!--
На рисунке показаны три части GUA: сначала это префикс глобальной маршрутизации, затем идентификатор подсети, затем идентификатор интерфейса. Первые три бита префикса глобальной маршрутизации — 001. Диапазон первого шестнадцатеричного текста составляет от 0010 0000 0000 (2000) до 0011 1111 1111 1111 1111 (3fff)
-->
...
<!--
На графике показаны три части GUA: во-первых, глобальный префикс маршрутизации, который имеет длину 48 бит, затем идентификатор подсети, который имеет длину 16 бит, затем, наконец, идентификатор интерфейса, который имеет длину 64 бит. Префикс маршрутизации A /48 + 16-битный идентификатор подсети = префикс /64.
-->
...
<!--
Физическая топология, показывающая два ПК, сервер, принтер, коммутатор и маршрутизатор. Он показывает, что связь по локальным IPv6-адресам канала не маршрутизируется вне сети.
-->
...
<!--
На рисунке показаны два маршрутизатора, соединенных по ссылке со своими LLA-адресами. Номер 1 с двунаправленной стрелкой, указывающей на каждый маршрутизатор, находится над каналом вместе с текстовыми сообщениями протокола маршрутизации. Компьютер подключен к маршрутизатору слева со стрелкой 2, указывающей от ПК на маршрутизатор. Текст под графиком гласит 1. Маршрутизаторы используют LLA соседних маршрутизаторов для отправки обновлений маршрутизации. 2. Узлы используют LLA локального маршрутизатора в качестве шлюза по умолчанию.
-->
