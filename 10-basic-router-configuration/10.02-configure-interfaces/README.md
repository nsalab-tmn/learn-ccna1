...
На этом этапе маршрутизаторы имеют свои базовые конфигурации. Следующий шаг — настройка их интерфейсов. Без этого маршрутизаторы не будут доступны конечным устройствам. На маршрутизаторах Cisco используется множество разных интерфейсов. Например, маршрутизатор Cisco ISR 4321 оснащен двумя интерфейсами Gigabit Ethernet:

* **GigabitEthernet 0/0/0 (G0/0/0)**
* **GigabitEthernet 0/0/1 (G0/0/1)**

Настройка интерфейса маршрутизатора очень похожа на управление SVI на коммутаторе. Выполняем следующие команды:
...
**Примечание:** На включенном интерфейсе маршрутизатора должны отображаться сообщения, подтверждающие включенную связь.

Несмотря на то, что команда **description** не требуется для включения интерфейса, рекомендуется ее использовать. Это поможет устранять неполадки в
производственных сетях, предоставляя информацию о типе подключенной сети. Если интерфейс подключен к интернет-провайдеру или оператору услуги, команда **description** поможет ввести стороннее подключение и контактную информацию.

**Примечание**: Текст описания может содержать не более 240 символов.

Команда **no shutdown** активирует интерфейс и аналогична включению интерфейса. Чтобы активировать физический уровень, интерфейс должен быть также подключен к другому устройству (коммутатору или маршрутизатору).
...
**Примечание:** Обратите внимание на сообщения, информирующие, что G0/0/0 и G0/0/1 включены.

<!-- 10.2.3 -->
## Проверка конфигурации интерфейса

Проверить конфигурации интерфейса можно несколькими командами. Наиболее полезные из них: **show ip interface brief** и **show ipv6 interface brief**, как показано в примере:
...
В таблице приведены самые популярные **show** команды для проверки конфигурации интерфейса:
