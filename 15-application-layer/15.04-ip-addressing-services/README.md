...
Для конкретных уровней приложений разработали протоколы, облегчающие получение адресов для сетевых устройств. Эти сервисы необходимы, потому что было бы очень сложно запомнить IP-адреса вместо URL-адресов или вручную настроить все устройства в сети среднего и большого размера. В первой теме модуля вы уже обозревали эти протоколы. В этом разделе более подробно рассматриваются службы IP-адресации, DNS и DHCP.

В сетях передачи данных устройства помечаются по числовым IP-адресам для отправки и получения данных. Доменные имена созданы, чтобы преобразовать числовой адрес в простое и легко запоминаемое имя.

В Интернете полные доменные имена (fully-qualified domain names - FQDN), такие как [http://www.cisco.com](http://www.cisco.com/), гораздо проще запомнить, чем 198.133.219.25 — фактический числовой адрес этого сервера. Если Cisco решит изменить числовой адрес [www.cisco.com](http://www.cisco.com/), он будет прозрачен для пользователя, поскольку доменное имя остается прежним. Новый адрес будет просто привязан к существующему имени домена без нарушения связи с сервером.
...
**Шаг 1:** Пользователь вводит полное доменное имя в поле адреса приложения браузера.
...
На DNS-серверах хранятся различные типы записей ресурсов, используемые для разрешения имен. Они содержат имя, адрес и тип записи. К некоторым типам записи относятся:
...
При настройке сетевого устройства указывают один или несколько адресов DNS-серверов, которые клиент может использовать для разрешения имен. Обычно адреса DNS-серверов предоставляет интернет-провайдер (ISP). Когда пользовательское приложение запрашивает подключение к удаленному устройству по его имени, клиент DNS опрашивает сервер имен, чтобы преобразовать имя в числовой адрес.

В операционных системах компьютеров обычно есть утилита Nslookup, которая позволяет пользователю вручную опрашивать серверы для разрешения имен узлов. Этой утилитой также можнонаходить проблемы разрешения имен и проверять состояние серверов имен.

На рисунке показано, что после выполнения команды **nslookup** выводится DNS-сервер по умолчанию, настроенный для этого узла. В **nslookup** командной строке можно ввести имя узла или домена. У утилиты nslookup много параметров для расширенного тестирования и проверки процесса DNS.
...
Служба протокола динамической конфигурации узла (Dynamic Host Configuration Protocol, DHCP) для IPv4 автоматизирует назначение адресов, масок подсети, шлюзов и других сетевых параметров IPv4. Это называется динамической адресацией. Альтернатива динамической адресации — статическая адресация. При её использовании администратор сети вручную вводит данные IP-адресов на узлах.

При подключении узла к сети устанавливается связь с DHCP-сервером и запрашивается адрес. DHCP-сервер выбирает адрес из заданного диапазона (пула), и назначает его (сдает в аренду) узлу.

Назначать адреса с помощью DHCPВ предпочтительно, если сеть крупная или часто меняет пользователей. Могут появиться новые пользователи, которым нужно подключиться к сети. А другим пользователям могут установить новые компьютеры, которые также требуют подключения. Вместо использования статической адресации для каждого соединения намного эффективнее автоматически назначать IPv4-адреса с помощью DHCP.

DHCP может выделять IP-адреса на настраиваемый период времени, называемый периодом аренды. Это важный параметр DHCP. По истечении срока аренды сервер получает сообщение DHCPRELEASE, адрес возвращается в пул DHCP для повторного использования. Пользователи могут свободно переходить на другое место и снова подключиться к сети по DHCP.
...
В большинстве сетей используется и DHCP, и статическая адресация. DHCP используют для узлов общего назначения, таких как конечные пользовательские устройства. Статическая адресация применяется для сетевых устройств: шлюзов, коммутаторов, серверов и принтеров.

DHCP для IPv6 (DHCPv6) предлагает аналогичные сервисы для клиентов IPv6. Отличие в том, что DHCPv6 не предоставляет адрес шлюза по умолчанию. Он может быть получен только динамически с помощью сообщения «Ответ маршрутизатора» (Router Advertisement, RA).

<!-- 15.4.7 -->
## Принцип работы DHCP

Как показано на рисунке,  когда устройство IPv4 с настроенным DHCP загружается или подключается к сети, клиент выполняет широковещательную рассылку сообщения обнаружения DHCP (DHCPDISCOVER), чтобы найти в сети все доступные серверы DHCP. Сервер DHCP отвечает сообщением с предложением DHCP (DHCPOFFER), которое разрешает клиенту арендовать адрес. Сообщение с предложением содержит назначаемые адрес IPv4 и маску подсети, адрес IPv4 DNS-сервера и адрес IPv4 шлюза по умолчанию. В предложении аренды также указывается ее срок.

![](./assets/15.4.7.svg)


Клиент может получить несколько сообщений DHCPOFFER, если в локальной сети более одного сервера DHCP. В таком случае клиент должен выбрать один из них. Для этого он отправляет сообщение с запросом DHCP (DHCPREQUEST), в котором клиент указывает конкретный сервер и предложение аренды, которые он принимает. Клиент также может запросить адрес, который ранее был присвоен ему сервером.

Если адрес IPv4, запрошенный клиентом или предлагаемый сервером, по-прежнему доступен, сервер возвращает сообщение с подтверждением DHCP (DHCPACK), что данный адрес предоставлен клиенту. Если предложение больше недействительно, выбранный сервер отвечает сообщением с отрицательным подтверждением DHCP (DHCPNAK). Если возвращено сообщение DHCPNAK, процесс выбора должен начаться повторно с отправкой нового сообщения DHCPDISCOVER. После того как клиент арендовал адрес, аренду необходимо будет продлить до истечения срока ее действия с помощью другого сообщения DHCPREQUEST.
...
<!-- 15.4.8 -->
<!-- ## Исследовательская работа: изучение работы DNS [(описание в PDF)](./assets/15.4.8-lab---observe-dns-resolution_ru-RU.pdf)
