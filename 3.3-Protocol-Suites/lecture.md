...
<!--
На рисунке показаны три различных уровня, используемых для описания того, что происходит при общении лицом к лицу.  Нижний уровень, обозначенный физическим уровнем, показывает два человека, обменивающегося сообщением.  Средний уровень, помеченный уровнем правил, содержит список используемых протоколов бесед, включая: использование общего языка; ожидание хода; и сигнал после завершения.  Верхний уровень помечен уровнем содержимого и содержит сообщение: Где кафе?  Текст внизу гласит: Наборы протоколов - это наборы правил, которые работают вместе, чтобы помочь решить проблему.
-->
...
<!--
Рисунок представляет собой таблицу с текстом под ней. Таблица состоит из четырех строк и пяти столбцов. Первый заголовок столбца - имя уровня TCP/IP и считывает сверху вниз: приложение, транспорт, Интернет и доступ к сети. Второй заголовок столбца — TCP/IP. Протоколами приложений являются HTTP, DNS, DHCP и FTP. TCP и UDP являются протоколами транспортного уровня. Интернет-протоколы IPv4, IPv6, ICMPv4 и ICMPv6. Протоколами сетевого доступа являются Ethernet, ARP и WLAN.  Третий заголовок столбца - ISO. Протоколы приложений ACSE, ROSE, TRSE и SESE.  Транспортные протоколы: TP0, TP1, TP2, TP3 и TP4.  Интернет-протоколы являются CONP/CMNS и CLNP/CLNS.  Протоколы сетевого доступа: Ethernet, ARP и WLAN.  Четвертый заголовок столбца - AppleTalk. Протокол прикладного уровня - AFP Транспортные протоколы: ATP, AEP, NBP и RTMP.  Протокол Интернета это  AARP. Протоколы сетевого доступа: Ethernet, ARP и WLAN. Пятым заголовком столбца является Novell Netware. Протокол прикладного уровня - NDS. Транспортный протокол - SPX Протокол Интернета - IPX. Протоколы доступа к сети: Etherent, ARP и WLAN. Текст ниже таблицы гласит: Internet Protocol Suite или TCP/IP - Это наиболее распространенный и релевантных пакетов протоколов, используемых сегодня. Пакет протоколов TCP/IP представляет собой открытый стандартный набор протоколов, поддерживаемый рабочей группой Internet Engineering Task Force (IETF). Протоколы взаимодействия открытых систем (OSI) - это семейство протоколов, разработанных совместно в 1977 году Международной организацией по стандартизации (ИСО) и Международным союзом электросвязи (МСЭ). Протокол OSI также включал семиуровневую модель, называемую эталонной моделью OSI. Эталонная модель OSI классифицирует функции ее протоколов. Сегодня OSI в основном известен своей многоуровневой моделью. Протоколы OSI в основном были заменены TCP/IP. AppleTalk - Краткосрочный фирменный пакет протоколов, выпущенный компанией Apple Inc. в 1985 году для устройств Apple. В 1995 году Apple приняла использование TCP/IP вместо AppleTalk. Novell NetWare - недолговечный фирменный пакет протоколов и сетевой операционной системы, разработанный компанией Novell Inc. в 1983 году с использованием сетевого протокола IPX. В 1995 году компания «Новелл» приняла использование TCP/IP вместо IPX.
-->
...
<!--
На рисунке показаны протоколы TCP/IP, используемые для отправки пакетов между веб-браузером хоста и веб-сервером. Топология сети показывает узел, подключенный к облаку Интернета с подключением к веб-серверу. Конверт, представляющий пакет, отображается, проходящий между Интернетом и сервером. Из пакета исходит информация о протоколах, используемых на каждом уровне.  Сверху вниз: уровень приложений и протокол передачи гипертекста (HTTP); транспортный уровень и протокол управления передачей (TCP); уровень интернета и интернет-протокол (IP); уровень доступа к сети и Ethernet.
-->
...
![](./assets/3.3.4.png)
<!-- /courses/itn-dl/aeece080-34fa-11eb-ad9a-f74babed41a6/af1f6234-34fa-11eb-ad9a-f74babed41a6/assets/2dbd74a1-1c25-11ea-81a0-ffc2c49b96bc.svg -->
...
<!--
На рисунке показаны уровни TCP/IP и связанные протоколы. На прикладном уровне: DNS - это протокол системы имен; DHCPv4, DHCPv6 и SLAAC являются протоколами конфигурации хоста; SMTP, POP3 и IMAP являются протоколами электронной почты; FTP, SFTP и TFTP являются протоколами передачи файлов; а HTTP, HTTPS и REST - протоколы веб и веб-сервисов. На транспортном уровне: TCP — это протокол, ориентированный на подключение, а UDP — протокол без подключения. На интернет-уровне: IPv4, IPv6 и NAT являются интернет-протоколами; ICMPv4, ICMPv6 и ICMPv6 ND являются протоколами обмена сообщениями; OSPF, EIGRP и BGP являются протоколами маршрутизации. На уровне доступа к сети: ARP — это протокол разрешения адресов, а Ethernet и WLAN — протоколы передачи данных.  Текст внизу гласит: TCP/IP - это набор протоколов, используемый в Интернете и сетях сегодняшнего дня. TCP/IP имеет два важных аспекта для поставщиков и производителей: открытый стандартный набор протоколов - это означает, что он свободно доступен для общественности и может использоваться любым поставщиком на их оборудовании или в их программном обеспечении. Каждый стандартный протокол принят отраслевыми компаниями и утвержден организацией по стандартизации. Использование стандартов в разработке и реализации протоколов гарантирует, что продукты от разных производителей будет успешно взаимодействовать между собой.
-->
