...
<!--
На рисунке показано сравнение полудуплексных и полнодуплексных коммуникаций. Верхний рисунок показывает полудуплексную связь. Коммутатор S1 подключен к коммутатору S2, стрелка, проходящая от S1 к S2, указывающей, что устройство может только отправлять или принимать данные.  Нижний рисунок показывает полнодуплексную связь. Коммутатор S1 подключен к переключателю S2, стрелки, указывающие в обоих направлениях, говорят, что устройство может отправлять И получать данные одновременно.
-->
...
**Примечание**: Примечание. Другие операционные системы, например Linux и OS X, в случае невозможности подключения к серверу DHCP не назначают сетевому интерфейсу IPv4-адрес.
