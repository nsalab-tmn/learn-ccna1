...
<!--
На диаграмме показана схема подсети для данного адреса 172.16.0.0/22 с 4 битами, заимствованными из узла для создания подсетей. Все четыре октета показаны в двоичном формате, за которым следует точечный десятичный формат для данного сетевого адреса и для нескольких созданных подсетей. Указан сетевой адрес в двоичном формате 10101100.00010000.000000 (сетевая часть выделена серым цветом) 00.00000000 (часть узла выделена фиолетовым цветом) = 172.16.0.0/22. Для подсетей, перечисленных ниже, первые 22 бита выделены серым цветом (сетевая часть), следующие 4 бита — синим цветом, а последние 6 битов — оставшаяся часть хоста, затененная фиолетовым цветом. Подсеть 0 — 10101100.00010000.00000000.00000000 = 172.16.0.0/26. Подсеть 1 — 10101100.00010000.0000000.01000000 = 172.16.0.64/26. Подсеть 2:10101100.00010000.00000000.10000000 = 172.16.0.128/26. Подсеть 3 - 10101100.00010000.00000000.11000000 = 172.16.0.192/26. Подсеть 4 — 10101100.00010000.00000001.00000000 = 172.16.1.0/26. Подсеть 5 — 10101100.00010000.00000001.01000000 = 172.16.1.64/26. Подсеть 6 — 10101100.00010000.00000001.10000000 = 172.16.1.128/26. Подсети 7 - 13 не показаны. Подсеть 14 является 10101100.00010000.00000011.10000000 = 172.16.3.128/26. Подсеть 15 является 10101100.00010000.00000011.11000000 = 172.16.3.192/26.
-->
...
<!--
На диаграмме показаны назначения подсетей для корпоративной топологии с пятью сайтами, подключенными к облаку поставщика услуг Интернета. На каждом сайте отображается маршрутизатор, подключенный к поставщику услуг Интернета, несколько серверов, общедоступные требования к адресации IPv4 и назначенный адрес подсети. Каждому подключению маршрутизатора к поставщику услуг Интернета также был назначен адрес подсети. Подключению корпоративного штаба присваивается подсеть 172.16.0.0/26, а сайту с 40 адресами присваивается подсеть 172.16.0.64/26. Соединению филиала 1 назначается подсеть 172.16.0.128/26, а сайту с 25 адресами присваивается 172.16.0.192/26. Соединению филиала 2 назначается подсеть 172.16.1.0/26, а сайту с 30 адресами назначается подсеть 172.16.1.64/26. Соединению филиала 3 назначается подсеть 172.16.1.128/26, а сайту с 10 адресами назначается подсеть 172.16.1.192/26. Соединению филиала 4 назначается подсеть 172.16.2.0/26, а сайту с 15 адресами назначается подсеть 172.16.2.64/26.
-->
...
В этом задании дается количество необходимых узлов. Определите маску подсети, поддерживающую указанное количество узлов. Введите свои ответы в двоичном, десятичном и префикс-формате в полях.
