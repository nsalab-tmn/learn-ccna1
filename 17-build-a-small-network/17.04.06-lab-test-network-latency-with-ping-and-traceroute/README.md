...
В первой части вам нужно вычислить задержку сети при обращении к нескольким сайтам, расположенным в разных странах. Эту процедуру можно использовать в корпоративной сети предприятия для определения базового уровня производительности.

**Шаг 1. Проверьте подключение**

Для проверки подключения отправьте следующие эхо-запросы с помощью команды **ping** на сайты регионального интернет-регистратора (Regional Internet Registry, RIR):
...
**Примечание.** Если для сайтов разрешены IPv6-адреса, при необходимости для перевода в IPv4-адрес можно использовать опцию -4. Для этого введите команду **ping -4 www.arin.net**.
...
2.  Используя команду **ping** с функцией подсчета, отправьте 25 эхо-запросов на узел назначения, как показано ниже. При этом в текущем каталоге будет создан текстовый файл с именем arin.txt. Он будет содержать результаты эхо-запросов с помощью команды **ping**.
...
3.  Выполните команду **ping** для остальных сайтов.
...
**Примечание.** Если для сайтов разрешены IPv6-адреса, при необходимости для перевода в IPv4-адрес можно использовать опцию **-4**. Введите команду **tracert -4 www.lacnic.net > traceroute_lacnic.txt**.
