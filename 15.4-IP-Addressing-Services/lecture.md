...
<!--
это рисунок с клиентом, который подключается к DNS-серверу через сеть, используя полное доменное имя, введенное в поле URL браузера, поскольку пользователям легче использовать имя веб-сайта.
-->
...
<!--
это рисунок с клиентом, отправляющим сообщение DNS-запроса на DNS-сервер для URL www.cisco.com
-->
...
<!--
это рисунок, изображающий сервер DNS, отвечающий на полное доменное имя с IP-версией 4 адреса веб-сайта на клиентском компьютере.
-->
...
<!--
это рисунок, показывающий ответное сообщение DNS-запроса, возвращаемое на клиентский компьютер с полным доменным именем и его IPv4 адресом.
-->
...
<!--
это рисунок с клиентским компьютером, использующим IP-адрес версии 4, чтобы связаться с сервером www.cisco.com через сеть.
-->
...
<!--
На рисунке показана иерархия DNS. Вверху находится домен корневого уровня с доменами верхнего уровня (Top-Level Domains, TLD), подключенными к основному домену корневого уровня. TLD - это .net, .edu, .com, .au, .co и другие домены верхнего уровня. Под доменом .com находится домен второго уровня: www.cisco.com, а под cisco.com — www.cisco.com, ftp.cisco.com и mail.cisco.com.
-->
...
<!-- 15.4.5 -->
## Проверка синтаксиса - команда nslookup

Попрактикуйтесь в вводе команды nslookup как в Windows, так и в Linux

<!-- 15.4.6 -->
## Протокол динамической настройки сетевого узла (Dynamic Host Configuration Protocol, DHCP)
...
<!--
На рисунке изображен сервер DHCP ISP, подключенный к Интернету с тремя маршрутизаторами ISP с обозначением ISP1, ISP2, ISP#. Каждый маршрутизатор ISP подключен к другой сети. ISP1 подключается к беспроводной антенне мобильного работника, который является клиентом DHCP. ISP2 подключен к корпоративному сетевому маршрутизатору, который подключается к корпоративной локальной сети через собственный локальный DHCP-сервер, подключенный к коммутатору, подключенному к шести DHCP-клиентам. ISP3 подключен к беспроводному DHCP-серверу для домашней и сети малого предприятия, к которой подключены три DHCP-клиента.
-->
...
<!--
На рисунке показана лестница протокола с DHCP-клиентом с одной стороны и DHCP-клиентом с другой. DHCP-клиент отправляет DHCPDISCOVER сообщение DHCP-серверу. Сервер DHCP отправляет сообщение DHCPOFFER клиенту DHCP. DHCP-клиент отправляет сообщение DHCPREQUEST в ответ на DHCPOFFER с DHCP-сервера. Сервер DHCP отправляет сообщение DHCPACK обратно клиенту DHCP. Процесс называется DORA.
-->
