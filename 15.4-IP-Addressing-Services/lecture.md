...
В Интернете полные доменные имена (fully-qualified domain names - FQDN), такие как http://www.cisco.com, гораздо проще запомнить, чем 198.133.219.25, который является фактическим числовым адресом для этого сервера. Если Cisco решит изменить числовой адрес www.cisco.com, он будет прозрачен для пользователя, поскольку доменное имя остается прежним. Новый адрес будет просто привязан к существующему имени домена без нарушения связи с сервером.
...
[Открыть описание в PDF](./assets/15.4.8-lab---observe-dns-resolution_ru-RU.pdf)
