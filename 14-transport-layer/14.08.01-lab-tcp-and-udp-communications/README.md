...
Это упражнение по моделированию позволит понять основы протоколов TCP и UDP для более подробного их изучения в дальнейшем. Режим моделирования Packet Tracer дает возможность просматривать состояние различных PDU, когда они перемещаются по сети.

Описанные ниже шаги проведут вас через процесс запроса сервисов с помощью приложений, доступных на клиентском ПК. Вы изучите функциональные особенности протоколов TCP и UDP, мультиплексирования и функции номеров портов при определении локального приложения, запросившего или отправляющего данные. Packet Tracer не засчитывает это действие.
...
Уменьшите объем сетевого трафика, отображаемого при моделировании:
...
Теперь вы будете использовать **кнопку Захват/Вперед** на панели моделирования, чтобы наблюдать за протоколами, перемещающимися по сети.

**Примечание**: Кнопка Capture/Forward (Захват/Вперед) ' **\>\|** ' представляет собой маленькую стрелку, указывающую вправо с вертикальной линией рядом с ней.

1.  Нажмите кнопку **Capture/Forward (Захват/Вперед)** один раз. Все PDU передаются на коммутатор.
...
6.  Посмотрите на значение в поле «Флаги», которое находится рядом с полем «Окно». Значения справа от «b» представляют TCP-флаги, установленные для данного этапа обмена данными. Каждое из шести мест соответствует флагу. Наличие «1» в любом месте указывает, что флаг установлен. За один раз можно установить несколько флагов. Значения флагов показаны ниже.
...
3.  Нажмите кнопку **Capture/Forward** (Захват/Вперед). Нажмите на второй конверт PDU и откройте его.
...
8.  Закройте PDU и нажимайте кнопку **Capture/Forward**, пока вторая PDU не вернется на узел **FTP Client**. Единица PDU обозначена другим цветом.
...
6.  Закройте **PDU** и нажимайте кнопку **Capture/Forward (Захват/Вперед)** до тех пор, пока PDU не вернется на узел **E-mail Client** с флажком.
