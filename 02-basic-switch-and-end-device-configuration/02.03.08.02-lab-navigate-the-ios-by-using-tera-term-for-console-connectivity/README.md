...
**Примечание**: Роутеры, используемые в практических лабораторных работах CCNA, — это Cisco 4221 с Cisco IOS XE Release 16.9.3 (образ universalk9). В лабораторных работах используются коммутаторы Cisco Catalyst 2960 с Cisco IOS версии 15.0(2) (образ lanbasek9). Можно использовать другие роутеры, коммутаторы и версии Cisco IOS. В зависимости от модели устройства и версии Cisco IOS доступные команды и результаты их выполнения могут отличаться от тех, которые показаны в лабораторных работах. Правильные идентификаторы интерфейса можно найти в сводной таблице по интерфейсам роутеров в конце лабораторной работы.

**Примечание.** Убедитесь, что все настройки коммутатора и роутера удалены, и загрузочная конфигурация отсутствует. Если вы не уверены, обратитесь к инструктору.

## Необходимые ресурсы

-   1 роутер (Cisco 4221 с универсальным образом Cisco IOS XE версии 16.9.3 или аналогичным).
-   1 коммутатор (Cisco 2960 с ПО Cisco IOS версии 15.0(2) с образом lanbasek9 или аналогичная модель).
-   1 ПК (под управлением Windows с программой эмуляции терминала, например, Tera Term).
-   Инвертированный консольный кабель (DB-9–RJ-45) для настройки коммутатора или роутера через консольный порт RJ-45.
-   Кабель mini-USB для настройки руотера через консольный порт USB.
...
**Примечание.** При использовании адаптера USB–DB9 для подключения к порту COM может потребоваться установка драйвера для адаптера. Этот драйвер предоставляется изготовителем компьютера. Чтобы определить порт COM, используемый адаптером, выполните шаг 3 в части 3. Номер порта COM требуется для подключения к устройству под управлением Cisco IOS при помощи эмулятора терминала в шаге 2.
...
4.  Когда отобразятся выходные данные терминала, все будет готово к настройке коммутатора Cisco.
...
2. Часы можно настроить в привилегированном режиме EXEC. Перейдите в этот режим, введя **enable** в командной строке пользовательского режима.
...
Если вы используете роутер Cisco 4221 или другие устройства под управлением Cisco IOS с консольным портом mini-USB, то для доступа к консольному порту можно использовать кабель mini-USB, подключенный к порту USB на компьютере.
...
2. Включите роутер Cisco и компьютер.
...
Перед подключением ПК с Microsoft Windows к устройству под управлением Cisco IOS при помощи кабеля USB необходимо установить USB-драйвер. Его для соответствующего устройства под управлением Cisco IOS можно найти на сайте [www.cisco.com](http://www.cisco.com). Драйвер можно загрузить по ссылке:
...
**Примечание.** Эта ссылка относится к роутеру Cisco 1941. Однако драйвер консоли USB не зависит от модели устройства под управлением Cisco IOS. Этот драйвер работает только с роутерами и коммутаторами Cisco. По завершении установки драйвера USB необходимо перезагрузить компьютер.
...
Когда индикатор консольного порта USB загорится зеленым, он будет доступен.
...
3. щелкните **Порты (COM и LPT)**, чтобы развернуть древовидную структуру, щелкните правой кнопкой мыши на значок **Последовательный порт USB** и выберите **Обновить драйверы**;
...
7. запишите назначенный номер порта, который отображается вверху окна (в данном примере для связи с роутером используется порт COM 5) и нажмите **Закрыть**;

8. откройте программу Tera Term, утановите переключатель **Serial** (последовательное) и выберите соответствующий последовательный порт (в данном примере это **Port COM5: Cisco Serial (COM 5)**), который теперь должен стать доступным для связи с роутером, и нажмите **OK**.
...
2.  Назовите достоинства и недостатки использования последовательного консольного подключения по сравнению с консольным подключением USB к роутеру или коммутатору Cisco.

Введите ваш ответ здесь.

## Сводная таблица по интерфейсам роутеров

| Модель роутера | Интерфейс Ethernet № 1          | Интерфейс Ethernet № 2          | Последовательный интерфейс № 1 | Последовательный интерфейс № 2 |
|-----------------------|---------------------------------|---------------------------------|--------------------------------|--------------------------------|
...
**Примечание**. Чтобы определить конфигурацию роутера, можно посмотреть на интерфейсы и установить его тип и количество его интерфейсов. Перечислить все комбинации конфигураций для каждого класса роутеров невозможно. Эта таблица содержит идентификаторы для возможных комбинаций интерфейсов Ethernet и последовательных интерфейсов на устройстве. Другие типы интерфейсов в таблице не представлены, хотя они могут присутствовать в данном конкретном роутере. В качестве примера можно привести интерфейс ISDN BRI. Строка в скобках — это официальное сокращение, которое можно использовать в командах Cisco IOS для обозначения интерфейса.
