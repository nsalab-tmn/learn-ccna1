...
<!--
На диаграмме показан хост-компьютер ПК A по адресу 192.168.10.10, и  использование команды ping 10.1.1.10 из командной строки для отправки четырех эхо-запросов ICMP с исходным IP-адресом 198.168.10.10 и  IP-адресом назначения 10.1.1.10, который является узлом ПК B в другой сети.
-->
...
<!--
На диаграмме показан хост-компьютер B по адресу 10.1.1.0, отправляющий четыре эхо-ответа ICMP с исходным IP-адресом 10.1.1.10 и адресом назначения 198.168.10.10 в ответ на запрос от компьютера A по адресу 192.168.10.10.
-->
...
<!--
На рисунке показано, как маршрутизатор использует стандартный ping для проверки связи с  хостом, отправляя четыре последовательных эхо-запроса ICMP, исходящие из ближайшего к месту назначения интерфейса. Маршрутизатор R1 подключен к двум сетям: слева — 192.168.10.0/24 на интерфейсе G0/0/1 с адресом .1 и справа — к сети 209.165.200.224/30 на интерфейсе G0/0/0 с адресом .225. Последняя сеть подключена к R2, которая подключена к сети 10.1.1.0/24, к которой хост ПК B подключен с адресом .10. R1 отправляет ПК B четыре ICMP эхо-запросом с  IP-адресом источника 209.165.200.225 и  IP-адресом  назначения 10.1.1.10.
-->
...
При нажатии клавиши **Примечание:** Pressing **Enter**  принимаются указанные значения по умолчанию
...
<!--
На рисунке  показано, как маршрутизатор использует расширенную команду ping для проверки связи с  узла путем отправки четырех последовательных эхо-запросов ICMP с указанным IP-адресом источника. Маршрутизатор R1 подключен к двум сетям: слева — 192.168.10.0/24 на интерфейсе G0/0/1 с адресом .1 и справа — к сети 209.165.200.224/30 на интерфейсе G0/0/0 с адресом .225. Последняя сеть подключена к R2, которая подключена к сети 10.1.1.0/24, к которой хост ПК B подключен с адресом .10. R1 отправляет ПК B четыре эхо-запроса ICMP с IP-адресом источника 192.168.10.1 и  IP-адресом получателя 10.1.1.10.
-->
...
<!--
На рисунке показана разница между командой trace, запущенной на хосте под управлением Windows и маршрутизатором Cisco IOS. Топология сети состоит из ПК A, подключенного к коммутатору R1, подключенному к маршрутизатору R2, подключенному к маршрутизатору R3, подключенному к коммутатору, подключенному к ПК B. ПК A с IP-адресом 192.168.10.10 выдает следующую команду из командной строки Windows: C: \>: tracert 10.1.1.10. R1 выдает следующую команду из интерфейса командной строки Cisco IOS: R#traceroute 10.1.1.10.
-->
...
**Примечание:** Use **Ctrl-C** to interrupt a **tracert** in Windows.
...
<!--
На рисунке показана топология сети с IP-адресом интерфейсов маршрутизатора и командой traceroute, выданной маршрутизатором Cisco IOS.  Топология состоит из следующих устройств и сетей слева направо.  Коммутатор в сети 192.168.10.0/24 подключен к маршрутизатору R1 по интерфейсу с адресом .1. R1 подключается к маршрутизатору R2 по сети 209.165.200.224/30.  Интерфейс на R1 имеет адрес 0,225, а интерфейс на R2 имеет адрес 0,226.  R2 подключен к маршрутизатору R3 по сети 209.165.200.228/30.  Интерфейс на R2 имеет адрес 0,229, а интерфейс на R3 — 0,230.  R3 подключается к коммутатору, который подключен к хост-ПК B с адресом 10.1.1.10.  R1 выдает следующую команду трассировки из интерфейса командной строки: R1# traceroute 10.1.1.10.
-->
...
<!--
На диаграмме показана топология сети с IP-адресом интерфейсов маршрутизатора и расширенной командой traceroute, выданной маршрутизатором Cisco IOS.  Топология состоит из следующих устройств и сетей слева направо.  Коммутатор в сети 192.168.10.0/24 подключен к маршрутизатору R1 по интерфейсу с адресом .1. R1 подключается к маршрутизатору R2 по сети 209.165.200.224/30.  Интерфейс на R1 имеет адрес 0,225, а интерфейс на R2 имеет адрес 0,226.  R2 подключен к маршрутизатору R3 по сети 209.165.200.228/30.  Интерфейс на R2 имеет адрес 0,229, а интерфейс на R3 — 0,230.  R3 подключается к коммутатору, который подключен к хост-ПК B с адресом 10.1.1.10.  R1 выдает следующую команду трассировки из интерфейса командной строки: R1# traceroute. 
-->
