...
В этом задании вам нужно будет разделить сеть клиента будет на несколько подсетей. При создании схемы подсети учитывайте количество компьютеров каждой подсети и другие аспекты, например дальнейшее расширение узлов в сети.
...
Когда сетевые устройства и компьютеры будут настроены, проверьте сетевые подключения с помощью команды **ping**.
...
6.  Чтобы разделить сеть на подсети, биты из узловой части исходной маски сети заменяются битами подсети. Их количество определяет количество подсетей.
...
Настройте IP-адрес, маску подсети и настройки шлюза по умолчанию на **РС-A** и **РС-B**.

### Часть 3. Проверка сети и устранение неполадок

В части 3 вы проверите подключение сети с помощью команды **ping**.

1.  Проверьте, может ли PC-A установить связь со своим шлюзом по умолчанию.
   
    - ответьте на вопрос №21

2.  Проверьте, может ли PC-B установить связь со своим шлюзом по умолчанию. 
   
    - ответьте на вопрос №22

3.  Определите, может ли PC-A взаимодействовать с PC-B. 
