...
Сеть организации может использовать частные и публичные блоки IP-адресов.
...
Интранет и DMZ имеют свои собственные требования и проблемы в области подсетей.

Интрасеть использует частное пространство адресации IPv4. Это позволяет организации использовать любой частный сетевой адрес IPv4, включая префикс 10.0.0.0/8 с 24 битами узлов и более 16 миллионами узлов. Использование сетевого адреса с 24 битами узлов делает подсеть простой и гибкой. Это включает создание подсетей на границе октета с использованием /16 или /24.

Например, частный сетевой адрес IPv4 10.0.0.0/8 можно поделить на подсети с помощью маски /16. Как показано в таблице, в результате получится 256 подсетей с 65 534 узлами в каждой. Если организации требуется менее 200 подсетей, что позволяет некоторый рост, это дает каждой подсети более чем достаточно адресов узлов.
...
Другой вариант использования частного IP-адреса 10.0.0.0/8 — разделение на подсети с использованием маски /24. Как показано в таблице, в результате получится 65 536 подсетей с 254 узлами в каждой. Если организации требуется более 256 подсетей, то с помощью параметра /24 можно использовать 254 узла на подсеть.
...
В таблице на рисунке показаны особенности разделения на подсети с префиксом /24. Обратите внимание, на обратную зависимость между числом подсетей и числом узлов. Чем больше бит заимствовано для создания подсетей, тем меньше доступно бит в узловой части. Если требуется больше узлов, то нужно больше бит в узловой части, что приводит к уменьшению количества подсетей.

От количества адресов узлов в самой крупной подсети зависит, сколько бит нужно оставить в узловой части адреса. Как вы помните, два адреса использовать нельзя, поэтому фактически количество доступных адресов рассчитывается как 2^n-2.
...
В этом примере штаб-квартира оператора связи выделила адрес частной сети 172.16.0.0/22 (10 бит в узловой части) для филиала. Как показано на рисунке, в результате мы получаем 1022 адреса узлов.
...
Таким образом, первые 4 бита в узловой части можно использовать для создания подсетей. Это значит, что будут заимствованы два бита из 3-го октета и два бита из 4-го октета. Если заимствовать 4 бита, новая длина префикса будет /26 с маской подсети 255.255.255.192.
