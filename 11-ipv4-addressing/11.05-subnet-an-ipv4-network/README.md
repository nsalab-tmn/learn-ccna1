...
В предыдущей теме вы узнали несколько веских причин для сегментации сети, т. е. для разделение на подсети. Это критически важный навык для администрирования сети IPv4. Сначала это сложно, но с практикой становится намного легче.

Для создания IPv4-подсетей мы задействуем один или несколько битов узла в качестве бита сети. Для этого мы расширяем маску подсети. Мы заимствуем биты из узловой части адреса и создаем дополнительные биты для сети. Чем больше битов заимствовано из узла, тем больше подсетей можно создать. Чем больше битов заимствовано для увеличения числа подсетей, тем меньше узлов в подсети.

Сети проще всего разделить на границах октетов /8, /16 и /24. В таблице указаны эти длины префикса. Обратите внимание, что увеличение длины префикса сокращает число узлов в каждой подсети.
...
Предприятие может дальше разбить адрес 10.0.0.0/8 на подсети на границе октета /16, как показано в таблице. Это даст возможность предприятию определить 256 подсетей (то есть 10.0.0.0/16 – 10.255.0.0/16), каждая из которых сможет связать 65 534 узла. Обратите внимание, что первые два октета пределяют адрес сетевой части, а последние два октета — IP-адрес узла.
...
В приведенных примерах мы использовали биты узлов из общих префиксов сети /8, /16 и /24. Однако подсеть может заимствовать биты из любой позиции в узловой части для создания других масок.
...
Для каждого бита, заимствованного в четвертом октете, доступное количество подсетей удваивается с сокращением числа адресов узлов в подсети.
