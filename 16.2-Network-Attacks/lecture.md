...
<!--
На первом рисунке показано окно приглашения для входа в систему с именем пользователя, администратора и
пароль, ****.
-->
...
<!--
На третьей фигуре, обозначенной «Перенаправление порта», изображен злоумышленник, подключенный к
маршрутизатору. Справа от злоумышленника находятся дополнительные компоненты,
Источник: злоумышленник Назначение: A Порт: 22 Маршрутизатор находится в цепочке из четырех
маршрутизаторов. Четвертый маршрутизатор подключен к брандмауэру. Брандмауэр имеет два
дополнительных соединения. Один подключен к скомпрометированному ПК,
Источник: A, назначение: B, порт: 23, а другой — к многоуровневому коммутатору. Консольный
Многоуровневый коммутатор имеет два дополнительных соединения: одно к паре ПК и
другой к паре серверов. Второй сервер имеет описание, хост B с
дополнительными деталями, Источник: злоумышленник Назначение: B Порт: 23
-->
...
<!--
Четвертая цифра с надписью «Человек в середине» изображает ноутбук с надписью «Жертва»,
отправляющий сообщения. Ноутбук подключен к коммутатору с надписью S5. S5
подключен к маршрутизатору, помеченному как R1. R1 подключен ко второму маршрутизатору с меткой
R2 и R2 подключены к облаку. Справа от облака находится
злоумышленник. Ниже хакера находится веб-сервер. Стрелка, помеченная цифрой 1, идет от
ноутбука жертвы к злоумышленнику. Вторая стрелка, помеченная цифрой 2, идет
от злоумышленника на веб-сервер. Третья стрелка, помеченная цифрой 3, идет от
веб-сервер к злоумышленнику. Четвертая стрелка, обозначенная цифрой 4, идет от
злоумышленника до ПК жертв.
-->
...
<!--
This animation shows a Web server, www.QZXBANK.com, two internet users, two PCs, and a threat actor all connected to an internet cloud. As the animation plays a text box appears above the threat actor stating “I have infected computers across the internet with the DDoS code. I will activate them all now”. A series of network messages leave the treat actor’s PC toward the Internet. From the internet the messages are forwarded to the internet users. Text boxes appear above the internet users stating “DDoS code activated”. The internet users’ computers then begin to forward network messages back to the internet. From the internet the messages from both internet users are forwarded to the WWWBANK.COM web server. A message appears above the web server stating “I am overloaded with too much traffic”.
-->
